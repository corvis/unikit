...
from unikit.progress import ProgressState, ProgressTracker, TProgressState, task_progress_logger
...
class TaskProgressReporter:
    ...
    async def set_object(self, *objects: Any, key: str | None = None) -> None:
        ...
        if len(objects) > 0:
            ...
            if dataclasses.is_dataclass(obj):
                meta_dict = dataclasses.asdict(obj)  # type: ignore[call-overload]
                if isinstance(obj, ProgressState):
                    meta_dict.pop("failed_items", None)
                    meta_dict.pop("current_status_msg", None)
                    meta_dict["speed_items_per_sec"] = obj.speed
            elif isinstance(obj, dict):
                meta_dict = obj
            task_progress_logger.info(
                "Tracked task: %s",
                str(obj),
                extra={f"progress_{k}": str(v) for k, v in meta_dict.items()},
            )
    ...
    @overload
    def create_tracker_sync(self, *, report_every_x_updates: int = 5) -> TaskiqProgressTracker[ProgressState]:
        pass

    @overload
    def create_tracker_sync(
        self, progress_state: TProgressState, report_every_x_updates: int = 5
    ) -> TaskiqProgressTracker[TProgressState]:
        pass
